...
async function getMensData(){
    try{
        // console.log("ok")
        let res=await fetch(`${baseURL}kid/`);
        // console.log(res)
        if(res.ok){
...
mensCat.addEventListener("change",async (e)=>{
    ...
    try{
        let res=await fetch(`${baseURL}men/filter?category=${catValue}&color=${colValue}`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
...
mensCol.addEventListener("change",async (e)=>{
    ...
    let colValue=mensCol.value;
    try{
        let res=await fetch(`${baseURL}kid/filter?category=${catValue}&color=${colValue}`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
...
mensExc.addEventListener("click",async (e)=>{
    try{
        let res=await fetch(`${baseURL}kid/exc`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
...
mSearch.addEventListener("input", async(e)=>{
    ...
    try{
        let res=await fetch(`${baseURL}kid/search?q=${searchVal}`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
...
mSort.addEventListener("change", async(e)=>{
    ...
    try{
        let res=await fetch(`${baseURL}kid/sort?sort=${sortVal}`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
