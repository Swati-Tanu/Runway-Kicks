...
mensCol.addEventListener("change",async (e)=>{
    ...
    let colValue=mensCol.value;
    try{
        let res=await fetch(`${baseURL}women/filter?category=${catValue}&color=${colValue}`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
...
mensExc.addEventListener("click",async (e)=>{
    try{
        let res=await fetch(`${baseURL}women/exc`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
...
mSearch.addEventListener("input", async(e)=>{
    ...
    try{
        let res=await fetch(`${baseURL}women/search?q=${searchVal}`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
...
mSort.addEventListener("change", async(e)=>{
    ...
    try{
        let res=await fetch(`${baseURL}women/sort?sort=${sortVal}`);
        if(res.ok){
            let data=await res.json();
            displayItems(data);
        }
